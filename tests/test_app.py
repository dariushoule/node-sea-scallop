...
def test_unpack_test1():
    ...
    assert Path('tests/seas/test1/build-node23/test1_unpacked').exists()
    assert Path('tests/seas/test1/build-node23/test1_unpacked/raw_sea.blob').exists()
    assert Path('tests/seas/test1/build-node23/test1_unpacked/dist_bundle.js').exists()
    clear_unpacked_repacked("test1")
...
def test_repack_test1():
    ...
    shutil.copyfile(
        ...
        "tests/seas/test1/build-node23/test1_repacked.exe"
    )
    ...
    with Path('tests/seas/test1/build-node23/test1_unpacked/dist_bundle.js').open('wb') as f:
        f.write(dist_bundle)

    result = runner.invoke(app, ["repack", "tests/seas/test1/build-node23/test1_repacked.exe", 
                                 "tests/seas/test1/build-node23/test1_unpacked/dist_bundle.js"])
    assert result.exit_code == 0, result.output
    assert "Repacked successfully!" in result.stdout

    out = subprocess.check_output(
        ["tests/seas/test1/build-node23/test1_repacked.exe"], stderr=subprocess.STDOUT
    ).decode("utf-8")
